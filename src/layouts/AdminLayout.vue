...
const glass = ref(localStorage.getItem("glass") !== "off");
...
function setGlass() {
  ...
  localStorage.setItem("glass", glass.value ? "on" : "off");
}
function logout() {
    ...
    <el-aside width="238px" class="sidebar glass">
      ...
      >
        ...
        <el-menu-item index="/users"
          ><el-icon><User /></el-icon><span>用户管理</span></el-menu-item
        >
        <el-menu-item index="/articles"
          ><el-icon><Document /></el-icon><span>文章管理</span></el-menu-item
        >
        <el-menu-item index="/categories"
          ><el-icon><FolderOpened /></el-icon><span>分类管理</span></el-menu-item
        >
      </el-menu>
      ...
      <el-header class="topbar glass">
        ...
        <div class="header-actions">
          <el-tooltip content="切换明暗主题"
            ><el-button circle text @click="setTheme"
              ><el-icon><Moon v-if="!dark" /><Sunny v-else /></el-icon></el-button
          ></el-tooltip>
          <el-tooltip :content="glass ? '关闭毛玻璃' : '开启毛玻璃'"
            ><el-button circle text @click="setGlass"
              ><el-icon><Document /></el-icon></el-button
          ></el-tooltip>
          <el-dropdown @command="(command) => command === 'logout' && logout()"
            ><span class="user-trigger"
              ><el-avatar :size="32" :icon="UserFilled" />{{ userName
...
.menu :deep(.el-menu-item.is-active) {
  background: rgba(13, 125, 112, 0.13);
  color: var(--accent);
  font-weight: 650;
}
.side-footer {
